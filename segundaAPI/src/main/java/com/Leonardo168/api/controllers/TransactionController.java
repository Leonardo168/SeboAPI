...
public class TransactionController {
	...
	/*
	 * @GetMapping("/{userId}") public ResponseEntity <Object>
	 * getProduct(@PathVariable(value = "userId") UUID userId, @PageableDefault(page
	 * = 0, size = 10, sort = "transactionDate", direction =
	 * Sort.Direction.ASC)Pageable pageable){ Optional<Page<Object>>
	 * transactionModelOptional = transactionService.findByBuyerId(userId,
	 * pageable); if(!transactionModelOptional.get().isEmpty()) { return
	 * ResponseEntity.status(HttpStatus.OK).body(transactionModelOptional.get()); }
	 * Optional<List<Object>> productModelOptional =
	 * productService.findByVendorId(userId); return
	 * ResponseEntity.status(HttpStatus.NOT_FOUND).
	 * body("No Transaction found for user: " + userId); }
	 */
}
